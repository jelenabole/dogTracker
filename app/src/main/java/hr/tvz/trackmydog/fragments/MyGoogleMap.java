...
public class MyGoogleMap {
    ...
    private final int MAX_HOURS_PASSED_FOR_MARKER = 48; // longer not showing
    ...
    private void setMapSettings() {
        // set map style:
        try {
            map.setMapStyle(MapStyleOptions.loadRawResourceStyle(context, R.raw.map_without_transit));
        } catch (Exception ex) {
     ...
     *
     * startAlpha = starting opacity (alpha of the dog's main marker)
     *
     * @param locations - list of dog previous locations
     * @param color - dog color
     */
    void setDogTracks(List<Tracks> locations, int dogIndex, String color) {
        // if dog changed in the meantime, return:
        if (followDogIndex != dogIndex) return;
        float startAlpha = markers.get(dogIndex).getAlpha();
        Log.d(TAG, "starting alpha: " + startAlpha);
