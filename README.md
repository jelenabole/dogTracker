...
This is an Android application that can be used to track dogs with any type of Android device. It can keep track of multiple devices simultaneously, and calculate their traveled distance and duration. The minimum required SDK for user application is 7.1, while the location service needs at least 4.0.

<p align="center">🚧<img align="center" src="images/01-tracking-2.png" width="180">🚧</p>

This application was built as a part of a bachelor's thesis. :mortar_board: The whole system consists of the user's mobile application, service on a chip for sending dog's location information, and a Firebase BaaS (real-time database and cloud functions).


## Usage
To use this application, simply install the APK file onto your device and launch the application. To use it, Google Play Services is required. After launching the application, you need to log in and add at least one dog. Dogs can be added by installing the trackerService on any Android device, after which you will receive a key for connecting with it. After adding a new dog, all features can be tested - features are explained below under "App Features". Apart from maps and dog list view, there is a (currently non-functioning) settings menu, which holds basic user info and safe zones, without any other settings options. The pre-built APKs are in this repository in the /libs directory.


## Setting up your environment
To develop this app further, you need to install:
🚧 👷‍♀️ 🚧
<!--
...
* 🗺️📍 **Map** shows user and dogs' positions
  * by default, a map zooms out so that all known dog locations are visible
  * the opacity of dogs' markers depends on the time elapsed since the last location update
  * clicking on the dog marker displays the info bubble (image 2)
  * clicking on the dog's thumbnail starts tracking selected dog - zooms in on the dog, shows his tracks, the screen follows the dog until the tracking is interrupted (image 3)
...
* 🏡❗ **Safe zone and Notifications**
  * users can enter multiple addresses (with specified range) that can be assigned to a dog as a movement restriction (image 1)
  * if a dog gets out of this area, the user will receive a notification (in the system tray or in-app, as shown in image 2)
...
* :dog: **Dog details**
  * displays user-entered dogs' details that can be edited here (image 1)
...
* [Creating Google Map, its configuration, and styles](https://developers.google.com/maps/documentation/android-sdk/map)
